...
Using `policy_regret_bound()`, we can find the "safety floor" - the minimum regret we must accept given the quality of our evidence ($\delta$).
...
The "safety floor" answers: *"What's the worst-case mistake I could make due to imperfect causal knowledge?"*

- **Safety Floor = 2M$\times$$\delta$**: The minimum unavoidable risk from the information gap
  - If safety floor = 0.02 on a 0-10 scale, worst-case error is ±0.02 units
  - This is the "price" you pay for using observational data instead of an RCT

**Decision Rule:**

- **If safety floor < acceptable risk threshold**: Proceed with confidence
- **If safety floor > acceptable risk threshold**: Either:

  1. Improve your data/adjustment (reduce $\delta$)
  2. Conduct a randomized trial
  3. Accept the risk and document it clearly

**Example:** If your gene target could save 5 units of benefit, but the safety floor is 0.5 units, you have a 10:1 benefit-to-risk ratio – likely acceptable.
...
4. State the safety floor: "The policy regret bound is XX, implying worst-case error of ±XX units"

**Complete Statement Example:**  
*"Using IPTW adjustment (deficiency $\delta$ = 0.01), we estimate the knockout reduces target expression by 2.3 units (95% CI: [1.8, 2.8]). The negative control diagnostic passed (p = 0.24), and the safety floor is 0.02 units, well below our risk tolerance threshold."*
