...
- $\delta$ = deficiency (quantifies confounding)
...
> **What $\delta$ is here.** `estimate_deficiency()` returns a propensity-score
> total-variation **proxy**, not an identified estimate of the exact deficiency.
> Bounds below that are stated in terms of $\delta$ inherit that: they are only
> as good as the proxy standing in for it.
...
# Illustrative deployment heuristic
if (transfer_penalty < 5) {
  cat("✓ LOW additional decision risk on the chosen 0-100 utility scale.\n")
} else if (transfer_penalty < 10) {
  cat("⚠ MODERATE additional decision risk. Deployment may be reasonable with monitoring.\n")
} else {
  cat("✗ HIGH additional decision risk. Consider stronger design or randomized evidence before deployment.\n")
}
```
...
# Find an illustrative low-risk region for this utility scale
safe_region <- subset(frontier$grid, delta < 0.1)
cat(sprintf(
  "Illustrative low-risk region covers %.1f%% of confounding space\n",
...
These cutoffs are illustrative for the 0--100 utility scale in this vignette. In practice, calibrate them to the decision stakes.

| Check | Illustrative heuristic | Action if Failed |
|-------|-------------------------|------------------|
| Transfer penalty < 5 points | Small additional regret budget | Deployment may be reasonable |
| Transfer penalty in [5, 10) points | Material but possibly manageable residual risk | Deploy only with active monitoring |
| Transfer penalty >= 10 points | Large residual regret budget | Consider pilot RCT or narrower deployment |
| NC diagnostic falsified | Any | Do not deploy without more data or stronger assumptions |
...
1. Akdemir, D. (2026). Constraints on Causal Inference as Experiment Comparison. 
   DOI: 10.5281/zenodo.21877511. See `thm:policy_regret` (Policy Regret Transfer) and
