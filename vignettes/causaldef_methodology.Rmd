...
Figure 1: The Safety Floor Curve. This curve represents the "Price of Safety" formula:

$$ Regret \leq Regret_{oracle} + 2M\delta $$

*   **Regret**: The cost of making a suboptimal decision based on your data.
*   **Regret_oracle**: The regret you would have even with perfect data.
*   **2M$\delta$**: The additional risk you take by not running an RCT.
...
## Step 6: Policy Regret Bounds (The Safety Floor)

This is the most critical step. We calculate the **Safety Floor**.

$$ \text{Safety Floor} = 2M\delta $$
...
*   **Example**: If Stakes ($M$) = $10M and $\delta$ = 0.01, your Safety Floor is $200,000.
*   **Decision**: 

    *   If expected profit ($5M) >> Safety Floor ($200K) $\rightarrow$ **Signal is Clear**.
    *   If expected profit ($250K) $\approx$ Safety Floor ($200K) $\rightarrow$ **That's Gambling**.
...
# Calculate the Delta corresponding to our Safety Floor limit
# Suppose our max acceptable safety floor is 0.2 units, and M=10
# Safety Floor = 2 * M * delta  =>  delta = Safety Floor / (2M)
acceptable_safety_floor <- 0.2
M <- 10
delta_threshold <- acceptable_safety_floor / (2 * M)
...
The contour line represents the border of the **Safety Floor**.  
