...
causal_spec <- function(data, treatment, outcome, covariates = NULL,
  ...
  # Handle missing values - only check for complete cases on analysis vars
  # but keep all columns in the data for potential downstream use (e.g., mediators)
  analysis_vars <- all_vars
  complete_idx <- complete.cases(data[, analysis_vars, drop = FALSE])
